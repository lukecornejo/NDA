...
/*
  ...
  ******************************************************
  ...
  To complile
  Windows
c++ -I .\eigen SC_NDA1.1.cpp ioNDA1.1.cpp defNDA1.1.cpp -o SC_NDA1.1.exe -std=c++0x
  Linux
  
  hpc
c++ -I .\eigen -O3 SC_NDA1.1.cpp ioNDA1.1.cpp defNDA1.1.cpp -o SC_NDA1.1b.exe
-O
-O2
-O3 better
-Os
...
icpc -I .\eigen -O3 SC_NDA1.1.cpp ioNDA1.1.cpp defNDA1.1.cpp -o SC_NDA1.1a.exe
-O1
-O2
-O3 best
-xO
...
double res_bal, res_x, res_y, res_lbc, res_rbc, res_bbc, res_tbc;
int i_bal, i_xx, i_yy, i_bbc, i_tbc;
int j_bal, j_xx, j_yy, j_lbc, j_rbc;
// iteration data
...
void input(char fn[]) //
{
	...
	for (i=0; i<nl; i++) {
		if ( isdigit(matl[i]) ) kbc=iparse(matl,i,nl); // kind of BC
		if ( matl[i]==';' ) break;
	}
	...
	for (i=0; i<nl; i++) {
		if ( isdigit(matl[i]) ) {
			if ( p==0 ) {
				nbc=iparse(matl,i,nl); // kind of BC
				xbc_n=nbc;
				xbc_e=new double[nbc+1];
				bc_b=new double[nbc];
				bc_t=new double[nbc];
			}
			else if ( p<nbc ) 	xbc_e[p]=dparse(matl,i,nl); // Left   BC or Quadrant 1
			else if ( p<2*nbc ) bc_b[p-nbc]  =dparse(matl,i,nl); // Bottom BC or Quadrant 2
			else                bc_t[p-2*nbc]=dparse(matl,i,nl); // Right  BC or Quadrant 3
			p++;
		}
		if ( matl[i]==';' ) break;
	}
	...
	for (i=0; i<nl; i++) {
		if ( isdigit(matl[i]) ) {
			...
			else if ( p<nbc )   ybc_e[p]=dparse(matl,i,nl); // Left   BC or Quadrant 1
			else if ( p<2*nbc ) bc_l[p-nbc]  =dparse(matl,i,nl); // Bottom BC or Quadrant 2
			else                bc_r[p-2*nbc]=dparse(matl,i,nl); // Right  BC or Quadrant 3
			p++;
		}
		if ( matl[i]==';' ) break;
	}
...
void calcDiffusion()
{
	...
	// D_xP
	for (i=0; i<nx+1; i++) {
		for (j=0; j<ny; j++) D_xP[i][j]=D_x[i][j]-D_xT[i][j]*xe[i]; // X Grid
	}
	// D_yP
	for (i=0; i<nx; i++) {
		for (j=0; j<ny+1; j++) D_yP[i][j]=D_y[i][j]-D_yT[i][j]*ye[j]; // Y Grid
	}
	// D_xN
	for (i=0; i<nx+1; i++) {
		for (j=0; j<ny; j++) D_xN[i][j]=D_x[i][j]+D_xT[i][j]*xe[i]; // X Grid
	}
	// D_yN
	for (i=0; i<nx; i++) {
		for (j=0; j<ny+1; j++) D_yN[i][j]=D_y[i][j]+D_yT[i][j]*ye[j]; // Y Grid
	}
...
void NDAsolution()
{
	...
	for (j=0; j<ny; j++) {
		x[p]=phiB_L[j]; // Left   BC Flux
		p++;
	}
	for (i=0; i<nx; i++) {
		x[p]=phiB_B[i]; // Bottom BC Flux
		p++;
	}
	for (j=0; j<ny; j++) {
		x[p]=phiB_R[j]; // Right BC Flux
		p++;
	}
	for (i=0; i<nx; i++) {
		x[p]=phiB_T[i]; // Top   BC Flux
		p++;
	}
	...
	for ( j=0; j<ny; j++ ) {
		for ( i=0; i<nx; i++ ) {
			...
			//cout<<hy[j]*D_xN[i+1][j]/xe[i+1]<<" ";
			b[p]=hx[i]*hy[j]*s_ext[i][j];
			p++;
		}
	}
	// ++++++++ Periphery Cells ++++++++
	if ( nx==1 and ny==1 ) { // Single Cell
		...
		for ( i=1; i<nx-1; i++ ) { // Top
			A.insert(p,p-1)=-hy[0]*D_xN[i][0]  /xe[i];    A.insert(p,p-nx)         =-hx[i]*D_yN[i][0]/ye[0];
			A.insert(p,p+1)=-hy[0]*D_xP[i+1][0]/xe[i+1];  A.insert(p,N_c+2*ny+nx+i)=-hx[i]*D_yP[i][1]/ye[1];
			p++;
		}
		// Top Right Corner
		i=nx-1;
	...
	//err_lo.push_back(1.0);      // error in lo solution
	//num_lo.push_back(1.0); // number of lo iterations
	...
	for (j=1; j<ny; j++) {
		for (i=0; i<nx; i++) j_y[i][j+1]=-(D_yP[i][j]*phi[i][j]-D_yN[i][j-1])/ye[j];
	}
...
void output(char fn[])
{
	...
	if ( nx==1 and ny==1 ) { // Single Cell
		...
		(hy[j]*D_xN[i+1][j]/xe[i+1]+hy[j]*D_xP[i][j]/xe[i]+hx[i]*D_yN[i][j+1]/ye[j+1]+hx[i]*D_yP[i][j]/ye[j]+
		hx[i]*hy[j]*(sigmaT[i][j]-sigmaS[i][j]-nuF[i][j]*sigmaF[i][j]))*phi[i][j]
		-hy[j]*D_xN[i][j]*phiB_L[j]/xe[i]-hx[i]*D_yN[i][j]*phiB_T[i]/ye[j]-hx[i]*hy[j]*s_ext[i][j]);
		if ( res>res_mbal ) { res_mbal=res; i_mbal=i; j_mbal=j; }
		for ( i=1; i<nx-1; i++ ) { // Middle Cell
			res=abs(-hy[j]*D_xP[i+1][j]*phi[i+1][j]/xe[i+1]-hx[i]*D_yP[i][j+1]*phiB_T[i]/ye[j+1]+
			(hy[j]*D_xN[i+1][j]/xe[i+1]+hy[j]*D_xP[i][j]/xe[i]+hx[i]*D_yN[i][j+1]/ye[j+1]+hx[i]*D_yP[i][j]/ye[j]+
	...
	for (i=1; i<nx-1; i++) {
		for (j=0; j<ny; j++) {
			// First Moment X Grid
			res=abs(j_x[i][j]+(D_xP[i][j]*phi[i][j]-D_xN[i][j]*phi[i-1][j])/xe[i]);
			if ( res>res_x ) { res_x=res; i_xx=i; j_xx=j; }
		}
	}
	for (i=0; i<nx; i++) {
		for (j=1; j<ny-1; j++) {
			// First Moment Y Grid
			res=abs(j_y[i][j]+(D_yP[i][j]*phi[i][j]-D_yN[i][j]*phi[i][j-1])/ye[j]);
			if ( res>res_y ) { res_y=res; i_yy=i; j_yy=j; }
		}
	}
	res_lbc=0; res_rbc=0;
	res_bbc=0; res_tbc=0;
	for (j=0; j<ny; j++) { // Left and Right
		res=abs(FL[j]*phiB_L[j]-j_x[0][j]); // Left BC residual
		if ( res>res_lbc ) { res_lbc=res; j_lbc=j; }
		res=abs(FR[j]*phiB_R[j]-j_x[nx][j]); // Right BC residual
		if ( res>res_rbc ) { res_rbc=res; j_rbc=j; }
	}
	for (i=0; i<nx; i++) { // Bottom and Top
		res=abs(FB[i]*phiB_B[i]-j_y[i][0]); // Bottom BC residual
		if ( res>res_bbc ) { res_bbc=res; i_bbc=i; }
		res=abs(FT[i]*phiB_T[i]-j_y[i][ny]); // Top BC residual
		if ( res>res_tbc ) { res_tbc=res; i_tbc=i; }
	}
	...
	for (i=0; i<n_iterations; i++) {
		outfile<<setw(4)<<i<<setw(2)<<" "<<print_out(rho_si[i])<<
	...
	for (i=0; i<nx; i++) {
		...
		print_out(FB[i])<<print_out(FT[i])<<endl;
	}
	...
	for (j=0; j<ny; j++) {
		...
		print_out(FL[j])<<print_out(FR[j])<<endl;
	}
	...
	for (i=0; i<nx; i++) {
		...
		<<print_csv(FB[i])<<print_csv(FT[i])<<endl;
	}
	...
	for (j=0; j<ny; j++) {
		...
		<<print_csv(FL[j])<<print_csv(FR[j])<<endl;
	}
