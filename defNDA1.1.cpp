...
void initialize()
{
	...
	psi=new double*[nx+2];
	for (i=0; i<nx+2; i++) psi[i]=new double[ny];
	...
	for ( i=0; i<nx; i++ ) {
		for ( j=0; j<ny; j++ ) {
			D[i][j]=1/(3*sigmaT[i][j]); // Diffusion Coefficient. Isotropic scattering so sigma_tr = sigma_t
			cout<<sigmaT[i][j];
		}
	}
	for ( i=1; i<nx; i++ ) {
		for ( j=0; j<ny; j++ ) {
			D_x[i][j]=2*D[i-1][j]*D[i][j]/(D[i-1][j]*hx[i]+D[i][j]*hx[i-1]); // X Grid D
		}
	}
	for ( j=0; j<ny; j++ ) { // Top and Bottom
		D_x[0][j]=2*D[0][j]/hx[0];
		D_x[nx][j]=2*D[nx-1][j]/hx[nx-1];
	}
	for ( i=0; i<nx; i++ ) {
		for ( j=1; j<ny; j++ ) {
			D_y[i][j]=2*D[i][j-1]*D[i][j]/(D[i][j-1]*hy[j]+D[i][j]*hy[j-1]); // Y Grid D
		}
	}
	for ( i=0; i<nx; i++ ) { // Left and Right
		D_y[i][0]=2*D[i][0]/hy[0];
		D_y[i][ny]=2*D[i][ny-1]/hy[ny-1];
	}
